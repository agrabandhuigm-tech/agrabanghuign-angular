...
}else if(isGratuity==undefined)
{
  ...
  checkAge(item:any)
  {
    ...
    let dob=moment(item.dob,'DD/MM/YYYY').format('YYYY-MM-DD[T]HH:mm:ss')
    let dob2=new Date(dob)
   
    let age = currentdate.getFullYear() - dob2.getFullYear();
    const monthDiff = currentdate.getMonth() - dob2.getMonth();
    const dayDiff = currentdate.getDate() - dob2.getDate();
    if (monthDiff < 0 || (monthDiff === 0 && dayDiff < 0)) {
      age--;
    }
    console.log("name",item.nameEng)
    console.log("age",age)
