...
export class PersonalDetailsComponent implements OnInit {
  ...
  ngOnChanges(changes: any) {
    if (!changes.EmpDetails?.firstChange) {
      ...
      this.onDomicileOfRajasthan(this.personalDetails.value.domicileOfRajasthan)
...
//     })
  ...
  onDomicileOfRajasthan(data:any)
  {
    const janAadharControl = this.personalDetails.get('janAadharId');
    const memberId = this.personalDetails.get('memberId');
    if (data.value=='Y') {
      janAadharControl?.setValidators([Validators.required, Validators.minLength(10)]);
      memberId?.setValidators([Validators.required]);
    } else {
      janAadharControl?.clearValidators();
      memberId?.clearValidators();
    }
    janAadharControl?.updateValueAndValidity(); 
    memberId?.updateValueAndValidity(); 
    this.printValidations()
  }
  printValidations() {
    const validationList: { [key: string]: string[] } = {};

    Object.keys(this.personalDetails.controls).forEach((key) => {
      const control = this.personalDetails.get(key);
      validationList[key] = this.getValidations(control);
    });

    console.log('Validation List:', validationList);
  }
  getValidations(control: AbstractControl | null): string[] {
  ...
  findInvalidControls() {
    ...
    const controls = this.personalDetails.controls;
