    ...
    getRevisionflag()
    {
      ...
      this.api.postNewEmployee("getPensionRevertEmpDetails",data).subscribe((res:any)=>
      {
        if(res.data)
        {
          ...
          if(res.data?.reasonforrevision!=null)
          {
            let data=res.data?.reasonforrevision;
            if(data.includes(','))
            {
              this.reasonRevisonList=data.split(",");
            }else
            {
              this.reasonRevisonList.push(data)
            }
...
removeReason(i:any)
{
  this.reasonRevisonList.splice(i,1);
}
  }
